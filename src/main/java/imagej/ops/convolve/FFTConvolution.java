 ...
 * 
 *         TODO-NOTE: maybe there is another way to convolve images using
 *         imglib2 instead of having this class here ...
 */
public class FFTConvolution<T extends RealType<T>, K extends RealType<K>, R extends RealType<R>>
     ...
     */
    public static <T extends RealType<T>, K extends RealType<K>> FFTConvolution<T, K, T> create(
            final Img<T> img, final Img<K> kernel) {
     ...
     */
    public static <T extends RealType<T>, K extends RealType<K>, R extends RealType<R>> FFTConvolution<T, K, R> create(
            final Img<T> img, final Img<K> kernel,
    ...
    public static <T extends RealType<T>, K extends RealType<K>, R extends RealType<R>> FFTConvolution<T, K, R> create(
            ...
            final RandomAccessibleInterval<R> output,
            final ImgFactory<ComplexFloatType> factory) {
        return create(Views.extendMirrorSingle(img), img, Views.extendValue(
                kernel, Util.getTypeFromInterval(kernel).createVariable()),
                kernel, output, factory);
    }
    ...
    public static <T extends RealType<T>, K extends RealType<K>> FFTConvolution<T, K, T> create(
            ...
            final ImgFactory<ComplexFloatType> factory) {
        return create(img, imgInterval, kernel, kernelInterval,
                Views.interval(img, imgInterval), factory);
    }
    ...
    /**
     * @param complexConjugate - If the complex conjugate of the FFT of the
    ...
    public void run() {
        ...
        FFTMethods.dimensionsRealToComplexFast(
                FinalDimensions.wrap(newDimensions), paddedDimensions,
                fftDimensions);

        // compute the new interval for the input image
        final Interval imgConvolutionInterval =
                FFTMethods.paddingIntervalCentered(imgInterval,
        ...
        final Interval kernelConvolutionInterval =
                FFTMethods.paddingIntervalCentered(kernelInterval,
                        FinalDimensions.wrap(paddedDimensions));
    ...
    final public static <T extends RealType<T>, K extends RealType<K>, R extends RealType<R>> void convolve(
        ...
        final Img<ComplexFloatType> fftImg =
                FFT.realToComplex(imgInput, factory);
