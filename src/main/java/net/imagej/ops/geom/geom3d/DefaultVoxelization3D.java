...
public class DefaultVoxelization3D extends
AbstractUnaryFunctionOp<Mesh, RandomAccessibleInterval<BitType>> implements
Ops.Geometric.Voxelization
{
	...
	public RandomAccessibleInterval<BitType> compute1(Mesh input) {
		...
		RealPoint centerPoint = new RealPoint( ( maxPoint.getDoublePosition(0) + minPoint.getDoublePosition(0) ) / 2,
				( maxPoint.getDoublePosition(1) + minPoint.getDoublePosition(1) ) / 2,
				( maxPoint.getDoublePosition(2) + minPoint.getDoublePosition(2) ) / 2 );
		...
		//RealPoint minShift = new RealPoint( -1*minPoint.getDoublePosition(0), -1*minPoint.getDoublePosition(1), -1*minPoint.getDoublePosition(2) );
		Vector3D minShift = new Vector3D( minPoint.getDoublePosition(0), minPoint.getDoublePosition(1), minPoint.getDoublePosition(2) ); 
		...
		for( Facet f : dMesh.getFacets() )
		{
			...
			double[] maxSubBoundary = new double[]{Math.max(Math.max(v1.getX(), v2.getX()), v3.getX()) - minPoint.getDoublePosition(0),
			...
			for (indices[0] = (long)Math.floor(minSubBoundary[0] / stepSizes[0]); indices[0] < Math.floor(maxSubBoundary[0] / stepSizes[0]); indices[0]++) {
				for (indices[1] = (long)Math.floor(minSubBoundary[1] / stepSizes[1]); indices[1] < Math.floor(maxSubBoundary[1] / stepSizes[1]); indices[1]++) {
					for (indices[2] = (long)Math.floor(minSubBoundary[2] / stepSizes[2]); indices[2] < Math.floor(maxSubBoundary[2] / stepSizes[2]); indices[2]++) {
						...
						if(!ra.get().get())// Don't check if voxel is already filled
						{
	...
	{
		double[] vminArray = new double[3];double[] vmaxArray = new double[3];
		for (int q = 0; q <= 2; q++)
		{
