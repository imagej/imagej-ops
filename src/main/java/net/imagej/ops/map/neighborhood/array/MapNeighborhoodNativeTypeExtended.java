...
@Plugin(type = net.imagej.ops.Op.class, name = Ops.Map.NAME,
	priority = Priority.LOW_PRIORITY + 11)
	...
	implements Contingent, Cancelable
{
		...
		final ArrayImg<O, ?> output)
	{
		...
		if (numDimensions == 1) {
			...
			/* center	*/
			center = new FinalInterval(new long[] { spanPlus1 }, new long[] {
				maxSafe0 });
		}
			...
			intervals = new FinalInterval[] {
				...
				new FinalInterval(new long[] { 0, dim1 - span }, new long[] { max0,
					max1 }),
				/* left */
				new FinalInterval(new long[] { 0, spanPlus1 }, new long[] { span, max1 -
					span }),
				/* right */
				new FinalInterval(new long[] { dim0 - span, spanPlus1 }, new long[] {
					max0, maxSafe1 }) };
			/* center */
			center = new FinalInterval(new long[] { spanPlus1, spanPlus1 },
				new long[] { maxSafe0, maxSafe1 });
		}
		else {
			...
			intervals = new FinalInterval[] {
				...
				new FinalInterval(new long[] { 0, 0, 0 }, new long[] { max0, max1,
					span }),
				/* back */
				new FinalInterval(new long[] { 0, 0, dim2 - span }, new long[] { max0,
					max1, max2 }),
				/* top */
				new FinalInterval(new long[] { 0, 0, spanPlus1 }, new long[] { max0,
					span, maxSafe2 }),
				/* bottom */
				new FinalInterval(new long[] { 0, dim1 - span, spanPlus1 }, new long[] {
					max0, max1, maxSafe2 }),
				/* left */
			...
			/* center */
			center = new FinalInterval(new long[] { spanPlus1, spanPlus1, spanPlus1 },
				...
				public void run() {
					if (oobFactory == null) {
						ops.run(MapNeighborhood.class, Views.interval(output, interval),
							Views.interval(input, interval), shape, getOp());
					}
					else {
						ops.run(MapNeighborhood.class, Views.interval(output, interval),
							Views.interval(input, interval), shape, getOp(), oobFactory);
					}
				}
			}));
		...
		futures.add(threadService.run(new Runnable() {
			...
			@Override
			public void run() {
				ops.run(MapNeighborhoodNativeType.class, output, input, shape, getOp(),
					finalCenter);
			}
		}));
