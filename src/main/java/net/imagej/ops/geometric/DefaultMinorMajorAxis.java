...
@Plugin(type = GeometricOp.class, label = "Geometric: MinorMajorAxis",
	name = Geometric2D.MinorMajorAxis.NAME)
	...
	Geometric2D.MinorMajorAxis
{
	...
	@Override
	public Pair<Double, Double> compute(Polygon input) {
		List<RealLocalizable> points = new ArrayList<RealLocalizable>(input.getVertices());

		// Sort the RealLocalizables of P by x-coordinate (in case of a tie, sort by
		...
		return new ValuePair<Double, Double>(minorMajorAxis[0], minorMajorAxis[1]);
	}
